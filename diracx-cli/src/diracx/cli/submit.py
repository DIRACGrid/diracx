...
def register_submit(app: AsyncTyper):
    """Register the top-level submit command on the main app."""
    ...
    Examples:
      dirac submit "python my_script.py"
      dirac submit "python my_script.py" --sandbox config.json
    """,
    )
    async def submit(
        command: Annotated[
            str, typer.Argument(help="Shell command to run on the grid")
        ],
        sandbox: Annotated[
            list[Path],
