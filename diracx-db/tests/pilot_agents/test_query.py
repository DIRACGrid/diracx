...
async def test_search_pagination(populated_pilot_db):
    """Test that we can search for pilots in the database."""
    async with populated_pilot_db as pilot_db:
        # Search for the first 10 pilots
        total, result = await pilot_db.search([], [], [], per_page=10, page=1)
        assert total == N
        assert result
        ...
        total, result = await pilot_db.search([], [], [], per_page=10, page=2)
        assert total == N
        assert result
        assert len(result) == 10
        ...
        total, result = await pilot_db.search([], [], [], per_page=10, page=10)
        assert total == N
        assert result
        assert len(result) == 10
        ...
        total, result = await pilot_db.search([], [], [], per_page=50, page=2)
        assert total == N
        assert result
        ...
        # Invalid page number
        total, result = await pilot_db.search([], [], [], per_page=10, page=11)
        assert total == N
        assert not result

        # Invalid page number
        with pytest.raises(InvalidQueryError):
            result = await pilot_db.search([], [], [], per_page=10, page=0)

        # Invalid per_page number
        with pytest.raises(InvalidQueryError):
            result = await pilot_db.search([], [], [], per_page=0, page=1)
