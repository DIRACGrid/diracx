...
from diracx.backend.bl.auth.device_flow import InitiateDeviceFlowResponse
from diracx.backend.bl.auth.device_flow import do_device_flow as do_device_flow_bl
from diracx.backend.bl.auth.device_flow import (
...
async def do_device_flow(
    ...
    authorization_flow_url = await do_device_flow_bl(
        request_url=request.url.replace(query=""),
        auth_db=auth_db,
        user_code=user_code,
        config=config,
        available_properties=available_properties,
