...
from diracx.backend.bl.auth.token import legacy_exchange as legacy_exchange_bl
from diracx.backend.bl.auth.token import token as token_bl
from diracx.backend.bl.auth.utils import GrantType
...
async def legacy_exchange(
    ...
    try:
        token = await legacy_exchange_bl(
