...
@pytest.fixture
def create_pilots_and_get_secrets(normal_test_client: TestClient):
    # Add a pilot stamps
    ...
    for creds in r.json():
        ...
        credentials.append((r.json()["access_token"], stamp))

    return credentials


@pytest.fixture
async def create_logs(create_pilots_and_get_secrets, normal_test_client):
    for i, creds in enumerate(create_pilots_and_get_secrets):
        access_token, stamp = creds
    ...
    # Return only stamps
    return [el[1] for el in create_pilots_and_get_secrets]


@pytest.fixture
async def search(create_logs, normal_test_client):
    async def _search(
        ...
        r = normal_test_client.post(
            "/api/pilot_management/search/logs", json=body, params=params
        )
...
async def test_single_send_and_retrieve_logs(normal_test_client: TestClient):
    ...
    pilot_stamp, secret, _ = pilot_credentials_list[0].values()
    ...
    r = normal_test_client.post("/api/pilot_management/search/logs", json=data)
