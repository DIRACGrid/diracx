...
1. **Build stage**: Uses `ghcr.io/prefix-dev/pixi` to install a pixi environment from the lock file with `pixi install --frozen`.
...
Container images are self-contained — the Dockerfile copies source code and uses `pixi install --frozen` to resolve everything from the lock file. No separate base images or wheel-building steps are needed:
